...
export const ladeValidate = (value) => {
  return value ? i18n.global.t("registration.alerts.lade") : "";
};

export const phoneValidate = async (number, value) => {
  try {
    const { data } = await axios.get(
      `https://api.apilayer.com/number_verification/validate?number=${number}${value}`,
      {
        headers: {
...
export const handleImageUpload = (value) => {
  ...
  error =
    value.size > maxSize ? i18n.global.t("registration.alerts.sizeFile") : "";

  return error;
};

export const streetValidate = (value) => {
  return !value ? i18n.global.t("registration.alerts.street") : "";
};
