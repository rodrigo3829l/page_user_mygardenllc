...
<v-container class="pa-4">
    <v-row>
        <v-col cols="12" class="d-flex justify-space-between align-center">
            ...
            <div>
                <v-btn @click="markAllAsRead" variant="outlined" color="green" class="mr-3" outlined>
                    <v-icon left>mdi-check-circle</v-icon>
                    Mark All as Read
                </v-btn>
                <v-btn @click="deleteAllNotifications" variant="outlined" color="red" outlined>
    ...
    <v-row v-if="notifications && notifications.length === 0" class="text-center">
        <v-col cols="12" class="my-5">
            <v-icon size="80" class="green--text text--darken-3 mb-4">mdi-leaf</v-icon>
            <h2 class="text-h4 font-weight-bold mb-3 green--text text--darken-3">No notifications available 🌸</h2>
    ...
    <v-row v-for="(group, date) in groupedNotifications" :key="date">
        <v-col cols="12">
          ...
          /> -->
            <NotificationComponent v-for="notification in group" :key="notification._id" :notification="notification" @mark-as-read="handleMarkAsRead" @delete-notification="handleDeleteNotification" />
        </v-col>
    </v-row>
</v-container>
</template>
    ...
    computed: {
        ...
        groupedNotifications() {
            return this.notifications.reduce((groups, notification) => {
                const notificationDate = new Date(notification.createdAt).toLocaleDateString(); // Solo la fecha
    ...
    methods: {
        async handleMarkAsRead(notificationId) {
            try {
                ...
                const {
                    data
                } = await api.put(`/notifications/mark/${notificationId}`);
                if (data.success) {
