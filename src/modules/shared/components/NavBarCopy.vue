...
<v-navigation-drawer v-model="drawer" prominent>
    <v-list-item prepend-avatar="https://randomuser.me/api/portraits/men/78.jpg" title="John Leider" subtitle="ejemaplo de correo"></v-list-item>

    <v-divider></v-divider>
    <v-list-item v-for="(link, index) in links" :key="index" :prepend-icon="link.icon" :title="link.name" @click="$router.push({ name: link.to })" :value="link.to"></v-list-item>
    ...
    <div v-if="userStore.token">
        ...
        <v-list-item prepend-icon="mdi-cogs" @click="$router.push({ name: 'recomendations' })" title="Recomendaciones"></v-list-item>
    </div>
                ...
                <v-col v-if="width > pixels" cols="auto">
                    <div class="collapse navbar-collapse" id="navbarNav">
                        <ul class="navbar-nav ml-auto">
                            ...
                            <router-link v-if="userStore.token === null" :to="{ name: 'login-login' }" class="nav-link is-active">
                                <v-icon size="15" color="white">mdi-login</v-icon>{{ $t('navbar.loginString') }}
                            </router-link>

                            <router-link :to="{ name: 'recomendations' }" class="nav-link is-active">
                                Recomendaciones
                            </router-link>
...
export default {
    ...
    data() {
        return {
            links: [{
                    name: i18n.global.t('navbar.homeString'),
                    to: 'home-home',
                    icon: 'mdi-home'
                },
                {
                    name: i18n.global.t('navbar.servicesString'),
                    to: 'services-services',
                    icon: 'mdi-hammer-screwdriver'
                },
                {
                    name: i18n.global.t('navbar.projectsString'),
                    to: 'proyects-view',
                    icon: 'mdi-briefcase'
                },
                {
                    name: i18n.global.t('navbar.contactString'),
                    to: 'home-contact',
                    icon: 'mdi-email'
                },
                {
                    name: i18n.global.t('navbar.faqsString'),
                    to: 'home-answers',
