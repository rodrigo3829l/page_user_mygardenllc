    ...
    <v-row>
      ...
      <v-col cols="12" md="6">
        <v-list lines="two">
          ...
          <v-list-item>
            <v-list-item-title>{{
              $t("profile.pages.infoMyService.descriptionString")
            }}</v-list-item-title>
          ...
          <v-list-item>
            <v-list-item-title>{{
              $t("profile.pages.infoMyService.totalPriceString")
            }}</v-list-item-title>
            <v-list-item-subtitle>${{ servicePrice }}</v-list-item-subtitle>
          </v-list-item>
          <v-list-item>
            <v-list-item-title>{{
              $t("profile.pages.infoMyService.pendingPaymentString")
            }}</v-list-item-title>
            <v-list-item-subtitle>${{ pending }}</v-list-item-subtitle>
          </v-list-item>
          <v-list-item>
            <v-list-item-title>{{
              $t("profile.pages.infoMyService.statusString")
            }}</v-list-item-title>
      ...
      <v-col cols="12" md="6">
        ...
        <v-table density="compact">
          <thead>
            <tr>
              <th class="text-left">
                {{ $t("profile.pages.infoMyService.productString") }}
              </th>
              <th class="text-left">
                {{ $t("profile.pages.infoMyService.quantityString") }}
              </th>
              <th class="text-left">
                {{ $t("profile.pages.infoMyService.totalString") }}
              </th>
            </tr>
          </thead>
          <tbody>
      ...
      <v-col cols="12" md="6">
        <v-row>
          <v-col cols="12" md="6">
            <v-list>
              <h2>
                {{ $t("profile.pages.infoMyService.costBreakdownString") }}
              </h2>
              <v-list-item>
                <v-list-item-title>{{
                  $t("profile.pages.infoMyService.laborString")
                }}</v-list-item-title>
                <v-list-item-subtitle>${{ laborCost }}</v-list-item-subtitle>
              </v-list-item>
              <v-list-item>
                <v-list-item-title>{{
                  $t("profile.pages.infoMyService.machineryString")
                }}</v-list-item-title>
                <v-list-item-subtitle
                  >${{ machineryCost }}</v-list-item-subtitle
                >
              </v-list-item>
            </v-list>
          </v-col>
          <v-col cols="12" md="6">
            <v-list>
              <h2>
                {{ $t("profile.pages.infoMyService.assignedEmployeesString") }}
              </h2>
              <v-list-item
              ...
              >
                <v-list-item-title
                  >{{ employee.name }} {{ employee.apellidoP }}
                  {{ employee.apellidoM }}</v-list-item-title
                >
              </v-list-item>
            </v-list>
          </v-col>
    ...
    <v-dialog v-model="paymentDialog" max-width="600" persistent>
      <v-card>
        <v-card-title class="headline">{{
          $t("profile.pages.infoMyService.paymentFormString")
        }}</v-card-title>
        <v-card-text>
          <v-text-field
            v-model="cardName"
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="closePaymentDialog">{{
            $t("profile.pages.infoMyService.buttonClose")
          }}</v-btn>
          <v-btn color="blue darken-1" text @click="submitPayment">{{
            $t("profile.pages.infoMyService.buttonPay")
          }}</v-btn>
        </v-card-actions>
      </v-card>
        ...
        <v-col cols="12" sm="8" md="6">
          <v-card class="mx-auto" outlined>
            <v-card-title class="justify-center text-h5">
              ...
              >{{
                $t("profile.pages.infoMyService.shareOpinionString")
              }}</v-card-title
            >
            ...
            <v-card-text>
              <v-form>
                ...
                <div class="text-center mb-2">
                  {{ $t("profile.pages.infoMyService.yourRatingString") }}
                </div>
...
export default {
  ...
  methods: {
    async submitComment() {
      this.overlay = true;
      try {
        const { data: translate } = await axios.get(
          `https://api.mymemory.translated.net/get?q=${this.comment}?&langpair=es-ES|en-GB`,
        );
        ...
        if (category === 1) {
          pred =
            "Gracias por el buen comentario, trabajamos por ofrecer el mejor servicio";
        } else if (category == 0) {
          pred =
            "Sentimos si algo estuvo mal, lo tomaremos en cuenta para la proxima, gracias";
        } else {
    ...
    async getService() {
      try {
        const { data } = await api.get(
          `/schedule/scheduleservice/${this.$route.params.id}`,
        );
        console.log(data);
        ...
        if (data.scheduledService.status === "finish") {
          const { data } = await api.get(
            `/comment/getCommentByScheduledId/${this.$route.params.id}`,
          );
          console.log(data);
    ...
    async submitPayment() {
      ...
      this.errors.expMonth = !isMonthSelected
        ? this.$t("profile.pages.infoMyService.script.expirationMonthString")
        : "";

      const isYearSelected = this.expYear !== "";
      this.errors.expYear = !isYearSelected
        ? this.$t("profile.pages.infoMyService.script.expirationRequiredString")
        : "";

      this.errors.cardName =
        this.cardName.length < 3 ||
        !/^[a-zA-ZüÜáéíóúÁÉÍÓÚñÑ' ]+$/.test(this.cardName)
          ? this.$t("registration.alerts.firstName")
          : "";

      const isValidNumber = /^\d{16}$/.test(this.cardNumber);
      this.errors.cardNumber = !isValidNumber
        ? this.$t("profile.pages.infoMyService.script.cardNumberString")
        : "";

      const isValidCVV = /^\d{3,4}$/.test(this.cvv);
      this.errors.cvv = !isValidCVV
        ? this.$t("profile.pages.infoMyService.script.theCodeString")
        : "";
      ...
      try {
        ...
        toast.success(data.msg);
        const newService = await api.get(
          `/schedule/scheduleservice/${this.$route.params.id}`,
        );
