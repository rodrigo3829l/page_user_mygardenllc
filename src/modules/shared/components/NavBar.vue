<template>
  <nav
    class="navbar navbar-expand-lg navbar-light bg-green-darken-3 navbar-transition"
    clipped-left
  >
    <v-container fluid>
      <v-row align="center" justify="space-between">
        ...
        <v-col cols="auto" class="d-flex flex-column ml-2 mr-auto company-info">
          ...
          <span class="subtitle d-none d-sm-block"
            >Landscaping services and more.</span
          >
        </v-col>
            ...
            <v-card>
              <v-card-text>
                <div class="mx-auto text-center">
                  ...
                  <v-btn rounded variant="text" @click="profile"
                    >Your Account</v-btn
                  >
                  <v-divider class="my-3"></v-divider>
                  <v-btn rounded variant="text" @click="services"
                    >Your Services</v-btn
                  >
                  <v-divider class="my-3"></v-divider>
                  <v-badge
                    ...
                    color="red"
                  >
                    <v-btn rounded variant="text" @click="notifications"
                      >Notifications</v-btn
                    >
                  </v-badge>
                  <v-btn v-else rounded variant="text" @click="notifications"
                    >Notifications</v-btn
                  >
                  <v-divider class="my-3"></v-divider>
                  <v-btn rounded variant="text" @click="logout" color="red"
                    >Logout</v-btn
                  >
                </div>
              </v-card-text>
    ...
    <v-navigation-drawer v-model="drawer" temporary right>
      <template v-if="userStore.token">
        <v-list-item
          :prepend-avatar="
            userStore.image || 'https://cdn.vuetifyjs.com/images/john.jpg'
          "
        ...
        <v-container>
          <v-row>
            <v-col cols="10" class="d-flex align-center">
              <v-icon color="green-darken-3" left class="mr-2"
      ...
      <div v-if="userStore.token">
        <v-list-item
          @click="$router.push({ name: 'profile-profile' })"
          :title="$t('navbar.yourAccountString')"
        >
          <template v-slot:prepend>
        ...
        <v-list-item
          @click="$router.push({ name: 'profile-myservices' })"
          :title="$t('navbar.yourServices')"
        >
          <template v-slot:prepend>
      ...
      <template v-slot:append v-else>
        <div class="pa-2">
          <v-btn
            ...
            block
            @click="$router.push({ name: 'login-login' })"
          >
  ...
  <v-dialog v-model="surveyDialog" max-width="600px">
    <v-card>
      <v-card-title class="text-h5 font-weight-bold"
        >Customer Satisfaction Survey</v-card-title
      >

      <!-- Subtítulo ajustado -->
      <v-card-text>
        We noticed you recently completed a service. Please answer the following
      ...
      <v-card-item>
        ...
        <p>
          Was it easy to see your service information and the personalized
          prices?
        </p>
        ...
        <p>
          How satisfied are you with the notifications and updates about your
          services?
        </p>
  ...
  methods: {
    ...
    showNotification(message) {
      if (Notification.permission === "granted") {
        ...
        console.log(
          "No se pueden mostrar notificaciones, el permiso no ha sido concedido.",
        );
      }
    },
    ...
    async checkSurveyEligibility() {
      ...
      try {
        ...
        const { data: serviceData } = await api({
          method: "GET",
        ...
        if (
          ...
          serviceData.services.some((service) => service.pending !== 0)
        ) {
        ...
        const { data: satisfactionData } = await api({
          method: "GET",
    ...
    async submitSurvey() {
      ...
      try {
        ...
        const response = await axios.post(
          "https://sentiments-and-recomendatios.onrender.com/predict/satisfaction",
          datos,
        );
        ...
        switch (satisfactionLevel) {
          case 1:
            message =
              "Lamentamos no haber cumplido tus expectativas, trabajaremos para mejorar.";
            break;
          case 2:
            message =
              "Sentimos que no hayas quedado satisfecho, haremos mejoras.";
            break;
          case 3:
          ...
          case 4:
            message =
              "Nos alegra que hayas quedado satisfecho, seguiremos mejorando.";
            break;
          case 5:
            message =
              "¡Gracias por tu confianza! Nos complace haber superado tus expectativas.";
            break;
          default:
