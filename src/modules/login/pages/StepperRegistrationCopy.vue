          ...
          <v-stepper v-model="step">
            <v-stepper-header class="custom-stepper-header">
              <template v-for="n in 3" :key="`${n}-step`">
                <v-stepper-item :complete="step > n" :step="items[n - 1]" :value="n" :title="items[n - 1]"></v-stepper-item>
                <v-divider v-if="n !== 3" :key="n"></v-divider>
              </template>
            </v-stepper-header>
            <v-stepper-window>
              <v-stepper-window-item v-for="n in 3" :key="`${n}-content`" :value="n">
                <PersonalInformation v-if="n === 1" @updateData="updateData"></PersonalInformation>
                <LocationInformation v-else-if="n === 2" @updateData="updateData"></LocationInformation>
                <UserInformation v-else @updateData="updateData"></UserInformation>
              </v-stepper-window-item>
            </v-stepper-window>
            <VBtn color="error" @click="previo">Previo</VBtn>
            <VBtn v-if="step < 3" color="success" @click="siguiente">Siguiente</VBtn>
            <VBtn v-if="step >= 3" color="success" @click="signUp">Registro</VBtn>
          </v-stepper>
...
export default {
  ...
  methods: {
    ...
    siguiente() {
      ...
      if (Object.values(this.formData).every(value => value === '' || value === null || value === undefined)) {
        toast.warning('Por favor llene todos los campos');
        return;
      }
      if (Object.values(this.formData).some(value => value === '' || value === null || value === undefined)) {
        toast.warning('Por favor llene todos los campos');
        return;
      }
    ...
    signUp: async function() {
      ...
      if (!this.formData.agreeToTerms) {
        toast.error('Por favor acepte los terminos y condiciones para el registro')
        return
      }
      this.tittle = ''
      this.message = ""
  ...
  components: {
    PersonalInformation: defineAsyncComponent(() => import('../components/PersonalData.vue')),
    LocationInformation: defineAsyncComponent(() => import('../components/LocationData.vue')),
    UserInformation: defineAsyncComponent(() => import('../components/UserData.vue')),
    DialogMessage: defineAsyncComponent(() => import('../components/Dialog.vue'))
  }
}
</script>
