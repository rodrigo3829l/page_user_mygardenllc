            ...
            <v-card class="elevation-12">
                ...
                <v-stepper alt-labels v-model="step" :items="items" hide-actions>
                    <template v-slot:item.1>
                        ...
                        <v-row>
                            <v-col cols="12" md="6">
                                <v-text-field :label="$t('registration.personalInformation.firstNameString')" v-model="formData.nombre" required :error-messages="errors.nombre" variant="underlined" color="green-darken-3"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-text-field :label="$t('registration.personalInformation.lastNameString')" v-model="formData.apellidoPaterno" required :error-messages="errors.apellidoPaterno" variant="underlined" color="green-darken-3"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-text-field :label="$t('registration.personalInformation.middleString')" v-model="formData.apellidoMaterno" required :error-messages="errors.apellidoMaterno" variant="underlined" color="green-darken-3"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-text-field :label="$t('registration.personalInformation.dateString')" type="date" v-model="formData.fechaNacimiento" required :error-messages="errors.fechaNacimiento" variant="underlined" color="green-darken-3"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-select :label="$t('registration.personalInformation.genderString')" v-model="formData.sexo" :items="[$t('registration.personalInformation.maleString'), $t('registration.personalInformation.femaleString')]" required variant="underlined" color="green-darken-3"></v-select>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-row align="center">
                                    <v-col cols="4">
                                        <v-select v-model="formData.lada" :items="['+1 (USA)', '+52 (Mexico)']" :label="$t('registration.personalInformation.ladeString')" :error-messages="errors.lada" variant="underlined" color="green-darken-3"></v-select>
                                    </v-col>
                                    <v-col cols="8">
                                        <v-text-field :label="$t('registration.personalInformation.phoneString')" v-model="formData.phone" required :error-messages="errors.phone" type="number" variant="underlined" color="green-darken-3" :prefix="formData.lada ? formData.lada.split(' ')[0] : ''"></v-text-field>
                                    </v-col>
                                </v-row>
                            </v-col>
                        ...
                        <v-row>
                            <v-col cols="12" md="6">
                                <v-file-input @change="handleImageUpload" accept="image/*" :error-messages="errors.image" :label="$t('registration.personalInformation.selectPictureString')"></v-file-input>
                            </v-col>
                            <v-col cols="12" md="6">
                                <v-img v-if="formData.imagen" :src="formData.imagen" alt="Selected Image" class="image-preview" contain></v-img>
                        ...
                        <v-btn v-if="step > 1" @click="step--" color="red-darken-4" variant="text">
                            {{ $t('registration.buttons.buttonPreviousString') }}
                        </v-btn>
                        <v-btn v-if="step !== items.length" @click="validateInformation" color="green-darken-3" variant="text">
                    ...
                    <template v-slot:item.2>
                        ...
                        <v-row>
                            <v-col>
                                <v-row>
                                    <v-col cols="8" sm="9">
                                        <v-text-field v-model="formData.postalCode" :label="$t('registration.LocationDetails.postalString')" required type="number" :error-messages="errors.postalCode" variant="underlined" color="green-darken-3"></v-text-field>
                                    </v-col>
                                    <v-col cols="4" sm="3">
                                        <v-btn @click="validatePostalCode" color="green-darken-3" class="ma-2" small :loading="loadBtn" :disabled="loadBtn">
                                            {{ $t('registration.LocationDetails.buttonString') }}
                                        </v-btn>
                                    </v-col>
                                </v-row>

                                <v-autocomplete v-model="formData.city" :label="$t('registration.LocationDetails.cityString')" color="green-darken-3" required :disabled="true" :error-messages="errors.city" variant="underlined"></v-autocomplete>

                                <v-select v-model="formData.street" :label="$t('registration.LocationDetails.streetString')" required :disabled="!cpValidate" :error-messages="errors.street" variant="underlined" color="green-darken-3" :items="['Peachtree Street',
                      'Marietta Street', 
                      'Decatur Street', 
                      'Spring Street', 
                      'Auburn Avenue', 
                      ...
                      'Whittenham Clse'
                                ...
                                <v-text-field v-model="formData.neighborhood" :label="$t('registration.LocationDetails.neighborhoodString')" required :disabled="!cpValidate" :placeholder="$t('registration.LocationDetails.placeholderNeighboorhood')" :error-messages="errors.neighborhood" variant="underlined" color="green-darken-3"></v-text-field>

                                <v-text-field v-model="formData.houseNumber" :label="$t('registration.LocationDetails.houseString')" required :disabled="!cpValidate" :error-messages="errors.houseNumber" variant="underlined" color="green-darken-3" :placeholder="$t('registration.LocationDetails.placeholderNumHouse')" type="number"></v-text-field>

                                <v-btn v-if="step > 1" @click="step--" color="red-darken-4" variant="text">
                                    {{ $t('registration.buttons.buttonPreviousString') }}
                                </v-btn>
                                <v-btn v-if="step !== items.length" @click="validateDirecion" color="green-darken-3" variant="text" :disabled="!cpValidate">
                                    {{ $t('registration.buttons.buttonNextString') }}
                                </v-btn>
                            </v-col>
                        </v-row>
                    ...
                    <template v-slot:item.3>
                        ...
                        <v-row>
                            <v-col>
                                <v-text-field v-model="formData.userName" :label="$t('registration.userdata.userString')" color="green-darken-3" required :error-messages="errors.userName" variant="underlined" :placeholder="$t('registration.userdata.userNameString')"></v-text-field>
                                <!-- Este es el problema -->
                                <!-- <v-text-field
                  ...
                  ></v-text-field> -->
                                <v-text-field v-model="formData.email" :label="$t('registration.userdata.emailString')" required :error-messages="errors.email" variant="underlined" :placeholder="$t('registration.userdata.userEmailString')" type="email" color="green-darken-3"></v-text-field>

                                <v-text-field v-model="formData.password" :label="$t('registration.userdata.passwordString')" required :error-messages="errors.password" variant="underlined" color="green-darken-3" :placeholder="$t('registration.userdata.helloString')" :append-inner-icon="passwordVisible ? 'mdi-eye-off' : 'mdi-eye'" :type="passwordVisible ? 'text' : 'password'" outlined @click:append-inner="togglePasswordVisibility" @input="checkStrength"></v-text-field>

                                <div v-if="formData.password.length > 0">

                                    <v-progress-linear :color="formData.color" height="10" :model-value="formData.passwordStrength" striped></v-progress-linear>
                                ...
                                <v-text-field v-model="formData.confirmPassword" :label="$t('registration.userdata.confirmPasswordString')" required :error-messages="errors.confirmPassword" variant="underlined" color="green-darken-3" :placeholder="$t('registration.userdata.helloString')" :append-inner-icon="passwordVisibleConfirm ? 'mdi-eye-off' : 'mdi-eye'" :type="passwordVisibleConfirm ? 'text' : 'password'" outlined @click:append-inner="togglePasswordVisibilityConfirm"></v-text-field>

                                <v-checkbox v-model="formData.agreeToTerms" :required="true" :error-messages="errors.agreeToTerms" :label="$t('registration.userdata.descriptionString')" color="green-darken-3"></v-checkbox>

                                <v-btn v-if="step > 1" @click="step--" color="red-darken-4" variant="text">
                                    {{ $t('registration.buttons.buttonPreviousString') }}
                                </v-btn>
                                <v-btn :disabled="!formData.agreeToTerms" @click="validateUser" color="green-darken-3" variant="text">
...
<v-dialog v-model="dialogVisible" width="500">
    ...
    <v-sheet elevation="12" max-width="600" rounded="lg" width="100%" class="pa-4 text-center mx-auto">
        <v-progress-circular :size="70" :width="7" color="purple" indeterminate v-if="progress"></v-progress-circular>
        <div v-if="!progress">
            ...
            <div class="text-end">
                <v-btn class="text-none" :color="color" rounded variant="flat" width="90" @click="dialogVisible = false">
                    okey
                </v-btn>
            </div>
        </div>
    ...
    methods: {
        ...
        signUp: async function () {
            ...
            try {
                ...
                const {
                    data
                } = await api.post('/user/signup', datos);
