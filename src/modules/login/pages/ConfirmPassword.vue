            ...
            <v-card class="elevation-12">
                ...
                <v-card-text>
                    <v-form @submit.prevent="submit">
                        ...
                        <v-text-field :label="$t('passwordRecovery.confirm.passwordLabel')" v-model="password.value.value" :error-messages="password.errorMessage.value" variant="underlined" prepend-inner-icon="mdi-lock" :append-inner-icon="passwordVisible ? 'mdi-eye-off' : 'mdi-eye'" :type="passwordVisible ? 'text' : 'password'" @click:append-inner="togglePasswordVisibility" color="green-darken-3" outlined required @input="checkStrength"></v-text-field>

                        <div v-if="password.value.value">

                            <v-progress-linear :color="color" height="10" :model-value="passwordStrength" striped></v-progress-linear>
                ...
                <span v-else class="green--text">{{ $t('passwordRecovery.confirm.excellentPassword') }}</span> -->
                        ...
                        <v-text-field :label="$t('passwordRecovery.confirm.confirmPasswordLabel')" v-model="confirmPassword.value.value" :error-messages="confirmPassword.errorMessage.value" variant="underlined" prepend-inner-icon="mdi-lock" :append-inner-icon="passwordVisibleConfirm ? 'mdi-eye-off' : 'mdi-eye'" :type="passwordVisibleConfirm ? 'text' : 'password'" @click:append-inner="togglePasswordVisibilityConfirm" color="green-darken-3" outlined required></v-text-field>

                        <v-btn color="green-darken-3" block class="mt-4" type="submit" :disabled="dialog" :loading="dialog">
    ...
    setup() {
        ...
        const submit = handleSubmit(async (values) => {
            try {
                ...
                const {
                    data
                } = await api.post('/user/change', datos)
