          ...
          <v-card-text>
            <v-form class="text-center mx-auto">
              <div class="text-body-2">
                {{ $t("passwordRecovery.otp.messageMail") }}...{{ dataLocal }}
                <br />
              ...
              <div class="text-caption" v-if="countdown > 0">
                {{ $t("passwordRecovery.otp.resendString") }}
                {{ formatCountdown }}
              </div>
              <div class="text-caption" v-else>
                {{ $t("passwordRecovery.otp.dontReceive") }}
                <a
                  href="#"
                  @click.prevent="onClickResend"
                  style="color: #2e7d32"
                  >{{ $t("passwordRecovery.otp.resend") }}</a
                >
              </div> </v-form
            ><br />
            <v-divider></v-divider>
      ...
      try {
        ...
        setTimeout(async () => {
          ...
          if (data.success === false) {
            toast.warning(
              i18n.global.t("passwordRecovery.otp.alertInvalidCode"),
            );
          } else {
