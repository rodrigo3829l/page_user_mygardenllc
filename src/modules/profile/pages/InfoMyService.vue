              ...
              <v-card-item>
                ...
                <div class="d-flex justify-space-between">
                  ...
                  <span class="font-weight-bold"
                    >${{ servicePrice - pending }}</span
                  >
                </div>

                <div v-if="pending !== 0">
                  <div class="font-weight-bold">
                    You must pay at least 50% to start the service
                  </div>
                </div>
...
export default {
    ...
    async submitComment() {
      ...
      try {
        ...
        const { data: prediccion } = await axios.post(
          "https://sentiments-and-recomendatios.onrender.com/predict/comment",
          { comment: translation },
        );
        ...
        if (category === 1) {
          pred =
            "Thanks for the good comment, we strive to offer the best service";
        } else if (category == 0) {
          pred =
            "We apologize if something went wrong, we will take it into account for next time, thank you";
        } else {
  ...
  components: {
    CommentCard: defineAsyncComponent(
      () => import("@/modules/profile/components/CommentCard.vue"),
    ),
    PaymentCard: defineAsyncComponent(
      () => import("@/modules/profile/components/PayComponent.vue"),
    ),
  },
};
</script>
