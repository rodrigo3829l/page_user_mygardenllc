<template>
            ...
            <template v-else>
              <v-col cols="12" class="text-center">
                <v-alert color="green-darken-3" type="info" dismissible>
                  🌻 No services found. Try adjusting your search or filter
                  criteria. 🌿
                </v-alert>
              </v-col>
            </template>
    ...
    <v-card>
      ...
      <v-card-actions class="justify-end">
        ...
        <v-btn color="error" @click="confirmCancel">{{
          $t("profile.pages.myServices.yesString")
        }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="paymentDialog" width="auto" persistent>
    <PaymentCard
      :monto="selectedService.quote"
      :pendingAmount="selectedService.pending"
...
export default {
  name: "ServicesPage",
  components: {
    ServicesCard: defineAsyncComponent(
      () => import("../components/CardServiceUser.vue"),
    ),
    SkeletonServices: defineAsyncComponent(
      () => import("../components/SkeletonServices.vue"),
    ),
    CalendarCard: defineAsyncComponent(
      () => import("../components/CalendarCard.vue"),
    ),
    ...
    filteredServices() {
      ...
      if (this.selectedStatus && this.selectedStatus !== "all") {
        filtered = filtered.filter(
          (service) => service.status === this.selectedStatus,
        );
      }
    ...
    orderedServices() {
      ...
      if (this.selectedOrder === "latest_dates") {
        ordered = ordered.sort(
          (a, b) => new Date(b.dates.reserved) - new Date(a.dates.reserved),
        );
      } else if (this.selectedOrder === "oldest_dates") {
        ordered = ordered.sort(
          (a, b) => new Date(a.dates.reserved) - new Date(b.dates.reserved),
        );
    ...
    async reschedule(date) {
      try {
        ...
        const { data } = await api.put(
          `/schedule/rescheduleservice/${this.idService.id}`,
          datos,
        );
        console.log(data);
        if (!data.success) {
