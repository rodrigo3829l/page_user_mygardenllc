...
export default {
  ...
  methods: {
    ...
    navigateToSelectedTipo() {
      const selectedService = this.services.find(service => service.name === this.selectedTipo);
      if (selectedService) {
        const id = selectedService._id;
        this.$router.push({ name: 'services-Schedule', params: { id: id } });

      } else {
        console.log("No se encontró ningún servicio con el nombre seleccionado:", this.selectedTipo);
      }
    }
  },
  components: {
