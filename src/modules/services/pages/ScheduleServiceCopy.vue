                  ...
                  ></v-img>
                        ...
                        <v-row dense>
                            ...
                            <v-col cols="12" class="d-flex align-center justify-center my-3">
                                <video ref="video" width="150" height="100" autoplay v-if="cameraActive"></video>
                                <v-img v-else-if="scheduleImagePreview" :src="scheduleImagePreview" alt="Selected Image" max-width="150" height="100" class="my-3"></v-img>
                                <div v-else class="image-placeholder">Selected Image</div>
                            </v-col>
                        </v-row>

                        <v-btn color="green-darken-3" @click="capturePhoto" block v-if="cameraActive">
                            Capture Photo
                        </v-btn>
                ...
                <Swiper :modules="modules" :breakpoints="breakpoints" :space-between="20" :loop="true" :navigation="navigationOptions" :pagination="paginationOptions">
                    <SwiperSlide v-for="(comentario, i) in comentarios" :key="i">
                        <div class="card-content">
                            <ComentsUserCard userAvatar="mdi-account" :userName="
                    comentario.user.name +
                    ' ' +
                    comentario.user.apellidoP +
                    ' ' +
                    comentario.user.apellidoM
                  " :commentText="comentario.comment" :rating="comentario.rating" :service="comentario.service.name" />
                        </div>
                    </SwiperSlide>
                    <template v-slot:container-end>
                        <div class="swiper-button-prev"></div>
